...
// the requirement is really about: deleting an expense is not blocked by
// who is recorded as its enteredByMemberId.
import { test, expect } from "@playwright/test";
import { loginAsHouseholdMember } from "../lib/auth";

test("AC-003-b: a household member can delete an expense entered by the other household member", async ({ page }) => {
  await loginAsHouseholdMember(page);

  const categoryName = `E2E-003b-${Date.now()}`;
